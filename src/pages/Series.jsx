...
export default function Series() {
  ...
  const seriesAll = useMemo(() => catalog.filter((x) => x.type === "series"), []);

  const hero = useMemo(() => {
    const withHero = seriesAll.find((x) => x.hero);
    return withHero || seriesAll[0] || catalog[0];
  }, [seriesAll]);
  ...
  const series = useMemo(() => {
    if (activeGenre === "Semua") return seriesAll;
    return seriesAll.filter((it) => {
      const arr = it.genres || (it.genre ? [it.genre] : []);
      return arr.map(String).includes(activeGenre);
    });
  }, [seriesAll, activeGenre]);
    ...
    <PageShell>
      ...
      <div className="border-bottom border-secondary">
        ...
        >
          ...
          <Container className="py-5">
            {/* ✅ GENRE DI HERO (posisi kaya mockup) */}
            <div
              ref={genreRef}
              style={{
                position: "relative",
      ...
      <Container className="py-4">
        <div className="d-flex align-items-center justify-content-between mb-3">
          <div>
            <h5 className="fw-semibold m-0">Halaman Series</h5>
        ...
        <PosterHoverGrid items={series} onPlay={goPlay} onInfo={openItem} />
      </Container>
